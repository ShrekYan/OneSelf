...
| **重复代码** | 多处复制粘贴相同逻辑 | 抽取：工具函数/hook/公共组件放到 `apps/web/src/utils/` `apps/web/src/hooks/` |
| **复杂条件分支** | `if-else`/`switch` 太长 | 对象映射替代条件分支，抽取分支逻辑 |
| **命名不清晰** | `data`/`info`/拼音/看不懂的缩写 | 重命名为具体含义的名称 |
| **类型问题** | `any` 泛滥、重复定义 | 替换 `any`，共用类型统一放 `apps/web/src/types/` |
| **不合理导入** | `../../../../` 长相对路径 | 使用路径别名 `@/` |
...
```
# ❌ Before - 所有代码混在一起
apps/web/src/pages/ArticleList/index.tsx  # 800 行，包含一切
