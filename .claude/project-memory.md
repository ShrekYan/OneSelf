...
### 错误场景
要创建一个可以通过 `/command-name` 执行的命令，错误地将文件放在 `skills/` 目录下：`.claude/skills/workflow/SKILL.md`，执行时报错 `Unknown skill: workflow`。

### 正确规则
> **Claude Code 斜杠命令路径约定**：
> - ✅ `commands/` 目录 → `.claude/commands/{command-name}.md` → 可以通过 `/{command-name}` 直接执行
> - ❌ `skills/` 目录 → `.claude/skills/{skill-name}/SKILL.md` → **不能**通过 `/{skill-name}` 直接执行
...
- ❌ 错误路径：`.claude/skills/workflow/SKILL.md`
