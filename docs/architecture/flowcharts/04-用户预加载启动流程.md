...
flowchart TB
    ...
    INIT --> CHECK{预加载开关 USER_PRELOAD_ENABLED?}
    ...
    READ --> EMPTY{还有数据?}
    EMPTY -->|是| WRITE[Redis pipeline 批量写入 v{version}:*]
    WRITE --> LOOP
    EMPTY -->|否| SWITCH[原子切换：SET current-version = 新版本号]

    SWITCH --> CLEAN[异步清理旧版本数据（SCAN 分批删）]
    CLEAN --> DONE[服务启动完成 ✅]

    note over SWITCH,CLEAN "关键设计：
只读写新的 Key 空间
全部成功才切换 current-version 指针
切换后才清理旧版本
失败了什么都不改，旧版本继续服务"
```
