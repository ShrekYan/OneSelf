...
sequenceDiagram
    ...
    alt 攒批触发（1s 定时或满 10 条）
        C->>L: POST /api/v1/logs/batch
        L->>L: ValidationPipe 验证 DTO
        loop 每条日志
            L->>F: fs.appendFileSync 同步写入
            L->>G: UDP GELF 发送（异步，不阻塞）
        end
        L-->>C: 202 Accepted { received: n, timestamp: string }
    end
...
✅ 攒批 1s + 满 10 条触发：
1000 QPS → 平均 100 次/秒（按时间触发）或 100 次/秒（按数量触发）
   → 减少 10 倍请求数
   → 减少 10 倍网络开销
   → 日志服务 QPS 压力降 10 倍

💡 权衡：
增加最多 1s 延迟（或满 10 条立即发送）
换来 10 倍吞吐量提升
日志不是实时消息，完全可接受
```

**实际配置**（可通过环境变量调整）：
- `LOG_SERVICE_BATCH_SIZE`: 默认 10 条
- `LOG_SERVICE_BATCH_INTERVAL_MS`: 默认 1000ms (1秒)
- `LOG_SERVICE_BATCH_ENABLED`: 默认开启，可关闭改为立即发送
...
**实际实现细节**：
- `sendToGraylog()` 返回 Promise，但调用方不 await
- 使用 `.catch()` 处理错误，失败只打印警告
- UDP 套接字发送后立即关闭，不保持连接
...
```typescript
{
  received: number,     // 成功接收的日志条数
  timestamp: string     // 处理时间戳（ISO 格式）
}
```
...
| 组件                | 路径                                                             |
| ------------------- | ---------------------------------------------------------------- |
| LogServiceClient    | `services/backend/src/common/log-service/log-service-client.service.ts` |
...
| LoggerConfig        | `services/log-service/src/logger/logger.config.ts`                 |
| RequestLogMiddleware| `services/log-service/src/logger/request-log.middleware.ts`      |
| ResponseLogInterceptor | `services/log-service/src/logger/response-log.interceptor.ts` |
