...
sequenceDiagram
    ...
    participant B as 主业务服务<br/>backend:3000
    ...
    U->>F: 输入用户名密码
    F->>B: POST /auth/login
    Note over B: AuthClientService
    B->>A: 转发 POST /auth/login
    A->>DB: 查询用户信息
    DB-->>A: 返回用户数据(含密码哈希)
...
| 登录接口          | `services/backend/src/auth/auth.controller.ts`                  |
