---
title: Agent 规范化编写范式
slug: claude-code-agent-standard-paradigm
date: 2024-06-01
tags: [Claude Code, Agent, 规范化]
---

**系列前言**：本文是# Claude Code 工程化实战：从单兵到军团(保姆级教程)系列中关于 **Agent 规范化编写** 的一篇。之前的文章主要演示了 Agent 与 Skill 的"用法"，而本文重点讲解如何**规范化地编写 Agent**，以及它与 Skill 之间的边界与协作关系。
...
5. **编写 Capabilities**：分场景列出能力，明确边界。

6. **编写强制约束**：列出不可违反的硬规则，与 Skill 保持一致。

7. **编写开发完成验证**：列出任务完成后必须执行的检查项。

8. **编写 Knowledge Base**：说明预加载的 Skill 和需要读取的项目规则。

9. **编写 Response Approach**：定义标准工作流程。

10. **编写 Output Format**：定义输出结构和内容要求。

11. **编写 Example Interactions**：提供典型示例问题，帮助模型理解触发场景。

12. **编写 Completion Checklist**：列出完成前必须确认的事项。

## 8. 常见问题与最佳实践

### 8.1 何时需要新建 Agent vs 修改现有 Agent

| 场景                   | 建议                                           |
| ---------------------- | ---------------------------------------------- |
| 新增一个全新的角色     | 新建 Agent                                     |
| 扩展现有角色的能力边界 | 修改现有 Agent                                 |
| 新增一套规范           | 创建 Skill，让相关 Agent 通过 `skills` 引用    |
| 修改通用规范           | 修改 Skill，所有引用该 Skill 的 Agent 自动受益 |

### 8.2 Agent 文件过大怎么办

如果 Agent 文件超过 200 行，说明可能包含了应该放到 Skill 中的内容。解决策略：

1. **提取规范到 Skill**：把技术规范、编码规则、验证命令等放到 Skill 的 `reference/` 中。

2. **使用 `skills` 预加载**：通过 frontmatter 的 `skills` 字段引用 Skill，避免重复。

3. **拆分 Agent**：如果一个角色的职责过于宽泛，可以拆分为多个专注的 Agent。

### 8.3 如何确保 Agent 和 Skill 的规范一致

1. **Agent 的强制约束与 Skill 的 Constraints 保持一致**：Agent 中只列出最关键的约束，详细规则在 Skill 中定义。

2. **通过 Knowledge Base 声明读取规则**：明确 Agent 在执行任务时需要读取哪些 Skill reference 文件。

3. **定期审查**：定期检查 Agent 是否包含了应该放到 Skill 中的内容。

## 9. 总结

Agent 规范化编写的核心是 **"分层协作"**：

- **Skill 是规范层**：沉淀稳定、可复用的技术规范、编码规则、验证命令。

- **Agent 是角色层**：定义具体角色的行为方式、工作流程、输出格式和完成标准。

- **`skills` 字段是连接层**：让 Agent 可以预加载 Skill，避免重复定义。

通过这种分层设计，我们可以实现：

- **规范复用**：同一套规范可以被多个 Agent 引用。

- **规范演进**：修改 Skill 后，所有引用该 Skill 的 Agent 自动受益。

- **角色清晰**：每个 Agent 专注于自己的角色定位，不越界。

- **易于维护**：规范集中在 Skill 中，Agent 文件保持精简。

遵循本文的规范编写 Agent，你将拥有一个结构清晰、易于维护、规范一致的 Agent 体系。
