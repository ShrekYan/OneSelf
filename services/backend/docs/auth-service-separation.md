...
│   ├── src/
│   │   ├── auth/            # 保留代理控制器，转发请求到 auth-service
│   │   ├── common/          # 保留公共基础，移除授权组件
│   │   ├── shared/          # 新增：AuthClientService 调用认证服务
│   │   ├── article/         # 文章业务（不变）
│   │   ├── category/        # 分类业务（不变）
│   │   └── ...
│   └── package.json
│   └── docs/
│       └── auth-service-separation.md  ← 本文档
...
| 依赖项 | 处理方式 | 说明 |
|--------|----------|------|
| **数据库** | 共享同一个 MySQL | auth-service 只访问 `users` 和 `refresh_tokens` 表，无需分库 |
| **Redis** | 共享同一个 Redis | Key 已有前缀（`auth:`、`cache:`），不会冲突 |
| **JWT 密钥** | 相同配置 | auth-service 和 backend 使用同一个密钥签名/验证 |
| **依赖版本** | 严格一致 | 所有 NestJS、Prisma 等依赖版本与原项目保持一致 |

### 5.2 数据模型

**schema.prisma 只保留认证需要的两个模型：**
...
| 步骤 | 状态 |
|------|------|
| 1. 修改原 `auth.controller.ts` 添加配置开关 | ⏳ |
| 2. 同时启动 backend + auth-service 测试 | ⏳ |
| 3. 验证用户注册 / 登录 / 刷新 / 登出 / 业务认证 | ⏳ |
| 4. 修复问题 | ⏳ |
...
| 1. 确认测试全部通过 | ⏳ |
| 2. 修改配置开关，默认打开远程认证 | ⏳ |
| 3. 重启 backend 服务，全量切到 auth-service | ⏳ |
| 4. 监控错误日志 | ⏳ |
...
| 指标 | 单体架构 | 分离架构 | 说明 |
|------|----------|----------|------|
| 登录接口延迟 | ~5ms | ~6-10ms | 增加 1-5ms 内网 HTTP 调用，可接受 |
| 登录接口 QPS | ~1000 | ~1000+ | auth-service 可以独立扩容，线性提升总 QPS |
| 业务接口延迟 | - | 增加 ~1ms | 仅增加一次内网 HTTP 调用 |
| 可用性 | 整体绑定 | 独立可用 | auth-service 重启不影响业务服务 |
...
| 路径 | 说明 |
|------|------|
| `auth-service/src/app.module.ts` | 根模块，整合所有模块 |
| `auth-service/src/main.ts` | 启动入口 |
| `auth-service/src/auth/` | 认证核心模块 |
| `auth-service/src/authorization/` | 授权模块 |
| `auth-service/src/introspect/` | 内省接口模块 |
| `auth-service/src/users/` | 用户同步模块 |
| `auth-service/src/common/` | 公共基础设施 |
| `auth-service/prisma/schema.prisma` | 数据模型 |
| `auth-service/package.json` | 依赖配置 |

### backend（原项目）改造涉及文件

| 路径 | 改动 |
|------|------|
| `backend/src/app.module.ts` | 移除本地认证模块，添加 HttpModule |
| `backend/src/auth/auth.module.ts` | 改造为代理模式 |
| `backend/src/auth/auth.controller.ts` | 转发请求到 auth-service |
