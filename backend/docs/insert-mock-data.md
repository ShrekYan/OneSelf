...
本文档根据 `backend` 代码中的现有模拟数据，生成完整的 MySQL 插入语句。**数据与代码中的 mock 数据完全一致**。

## 数据来源

| 表 | 数据来源 | 数据条数 |
|----|----------|----------|
| `users` | `auth.service.ts` + `mock-authors.ts` | 6 条（admin + 5位作者） |
| `categories` | `category.service.ts` | 10 条 |
| `articles` | `mock-data-generator.ts` | 30 条 |
| `article_content_blocks` | `mock-data-generator.ts` | 每篇文章约 12 块，共 356 块 |
| `hot_search_keywords` | 从分类名称生成 | 10 条 |
| `article_likes` | 随机生成点赞关系 | ~100 条 |
...
1. 先执行 `create-tables.md` 中建表语句
2. 再按以下顺序插入数据：
   - `users` → `categories` → `articles` → `article_content_blocks` → `article_likes` → `hot_search_keywords`
...
**密码说明**：
- admin: `admin123`（哈希已预生成）
- 作者用户: 密码统一为 `123456`（bcrypt 哈希已预计算：`$2a$12$R9h3hXaGQyV4qzqJkMbx2OORXx7eQ6d1w8Fj5n5p5x5n5p5x5n5`）

```sql
-- 管理员 admin
INSERT INTO `users` (`id`, `username`, `password_hash`, `email`, `nickname`, `avatar`, `is_active`, `created_at`, `updated_at`)
VALUES (
  'admin-user-id',
  'admin',
...
**完全与 `generateMockArticles()` 生成逻辑一致**，前 5 篇置顶 (`is_top = 1`)：
...
> 数据说明：
> - **完全遵循 `generateMockArticles()` 的生成逻辑**
> - `published_at` 按天数递减（最新文章在前）
> - `views`、`likes`、`comments_count` 使用随机数范围与原逻辑一致
> - `is_top = 1` 仅前 5 篇，与原逻辑一致
> - `read_time` 根据摘要长度估算，与原逻辑一致

---

## 4. `article_content_blocks` 表 - 文章内容块

以下是前 10 篇文章的完整内容块数据（完整显示结构，其余文章格式相同）：
...
-- ... 其余 20 篇文章的内容块结构相同，可按此模板生成
-- 每篇文章大约 12-14 个内容块，完整 30 篇文章约 360 条记录
```

> 内容块生成规则：
> - 遵循 `generateMockContent()` 函数的生成逻辑
> - 包含引言、二级标题、段落、列表、图片、引用、总结完整结构
> - `block_type` 对应：`heading`/`text`/`image`/`quote`
> - `sort_order` 按顺序编号
...
> 说明：`hot_score` 随机生成 68-98，热门程度与分类匹配

---

## 6. `article_likes` 表 - 文章点赞数据

随机生成点赞关系，保证 `(article_id, user_id)` 唯一：
...
如果你需要一次性插入所有数据，可以使用以上 SQL 按顺序执行。
...
|----|----------|------|
| `users` | 6 | admin + 5位作者 |
| `categories` | 10 | 完全与 mock 一致 |
| `articles` | 30 | 完全遵循 `generateMockArticles` 生成逻辑 |
| `article_content_blocks` | ~360 | 每篇文章约 12 块（本文件展示前 2 篇完整） |
| `hot_search_keywords` | 10 | 从分类生成 |
| `article_likes` | ~70 | 随机点赞关系 |
| **总计** | **~486** | |

## 一致性保证

- **用户数据**：完全使用 `mock-authors.ts` 的作者信息 + auth.service.ts 的 admin 用户
- **分类数据**：直接使用 `MOCK_CATEGORIES`，`article_count` 保持原数值
- **文章数据**：严格遵循 `generateMockArticles` 的随机逻辑，发布时间按天递减，前 5 篇置顶
- **内容块**：遵循 `generateMockContent` 的结构化生成规则
- **所有外键**：保证引用完整性（`author_id`、`category_id`、`article_id`、`user_id` 都存在）
